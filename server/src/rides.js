...
function createRidesModule(io) {
  ...
  router.put('/:id/assign', authenticate, requireDriver, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  router.put('/:id/complete', authenticate, requireDriver, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  return { router, scheduleReminders, pool };
}
